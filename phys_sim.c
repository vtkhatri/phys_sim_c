...
void sim(int width, int height)
{
	...
	while(1) {
		...
		for (int y = height-1; y >= 0; y--) {
			...
			if (y==0) {
				if (space[width*4/3]==0) space[width/3] = 1; // put sand if there's nothing below @ 1/3 width
				if (space[width*5/3]==0) space[width/3] = 2; // put water if there's nothing below @ 2/3 width
			}
		}
